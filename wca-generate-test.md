...
- [Standard plan]{: tag-purple} Copy the unit test to a file in the high-level directory structure `src/test/java`. For example, copy the unit test file to `.../src/test/java/com/acme/modres/WCA_OrdersAlertFilter_Test.java`.
...
1. [Standard plan]{: tag-purple} Copy the unit test to a file in the high-level directory structure `src/test/java`. For example, copy the unit test file to `.../src/test/java/com/acme/modres/WCA_OrdersAlertFilter_Test.java`.
...
[Standard plan]{: tag-purple} You can start a unit test fix for tests generated by {{site.data.keyword.wca_short}}. Fixing unit tests is only available with the {{site.data.keyword.wcaej_short}} Visual Studio Code extension or Eclipse plug-in, version 1.1.0 and later.
...
**Requirements**:
- The Fix Unit Test feature is available for tests generated by {{site.data.keyword.wca_short}} that start with the prefix `WCA_`. Don't remove the `WCA_` prefix from the generated tests.
- The unit tests need to be stored in the `src/test/java` folder of a Java project or application.
